// components/auth/CodeOverlay.tsx
'use client'
  ...
  return (
    <OverlayWrapper onClose={onClose}>
      <h2 className="text-xl sm:text-2xl font-bold text-white mb-3 sm:mb-4 flex items-center justify-center gap-2 sm:gap-2.5">
        <span className="text-[#ff6b35]"><ShieldIcon /></span> Verification en deux etapes
      </h2>
      ...
      <AnimatePresence>
        {result.show && (
          <motion.div
            className={`mt-4 sm:mt-5 p-3 rounded-xl text-xs sm:text-sm font-semibold ${
              result.type === 'success'
