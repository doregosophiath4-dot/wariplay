...
export default function AnaMind() {
  ...
  const openWebSocket = useCallback((): Promise<void> => {
    return new Promise(async (resolve, reject) => {
      ...
      const wsUrl = proto + window.location.host + '/ws/amind'
      const ws = new WebSocket(wsUrl)
      wsRef.current = ws
      const t = setTimeout(() => {
        closeWebSocket()
        reject(new Error('Connexion serveur expirée'))
      }, 10000)
      ...
      ws.onerror = () => {
        clearTimeout(t)
        closeWebSocket()
        reject(new Error('Connexion serveur impossible'))
      }
      ...
      ws.onclose = () => {
        ...
        if (pendingRejectRef.current) {
          pendingRejectRef.current(new Error('Connexion serveur fermée'))
          pendingResolveRef.current = null
          pendingRejectRef.current = null
        }
      }
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(prev => {
        ...
        if (newTime <= 0) {
          clearTimer()
          setTimeout(() => {
            if (gameActiveRef.current && wsRef.current && wsReadyRef.current) {
              gameActiveRef.current = false
              if (pendingResolveRef.current) {
                const rf = pendingResolveRef.current
                pendingResolveRef.current = null
                pendingRejectRef.current = null
                rf({ success: false, message: 'Temps écoulé !', gain: 0 })
              }
            }
          }, 0)
        }
  ...
  const loadBalanceFromServer = useCallback(async () => {
    try {
      ...
      balanceRef.current = data.solde
    } catch (error) {}
  }, [])

  const sendBetToServer = useCallback(async (betAmount: number) => {
    setIsLoading(true)
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Erreur lors de la mise')
      }
      ...
      balanceRef.current = data.new_solde
      return data
    } catch (error) {
      throw error
    } finally {
  ...
  /* ── Logique du jeu ────────────────────────────────── */
  const endGame = useCallback((won: boolean, serverGain: number = 0) => {
    clearTimer()
    setIsWin(won)
    setLastGain(serverGain)
    gameActiveRef.current = false
    if (won) {
      setBalance(prev => { const nb = prev + serverGain; balanceRef.current = nb; return nb })
      setScorePop(true)
  ...
  const checkAnswer = useCallback(async () => {
    ...
    if (!input) {
      setMessage({ text: 'Veuillez entrer une réponse', type: 'error' })
      return
    }

    if (!currentHmacRef.current) {
      setMessage({ text: 'Session invalide, rechargez la page', type: 'error' })
      return
    }

    if (!wsRef.current || !wsReadyRef.current) {
      setMessage({ text: 'Connexion perdue', type: 'error' })
      return
    }

    gameActiveRef.current = false
    if (userInputRef.current) userInputRef.current.disabled = true
    ...
    try {
      const result: any = await new Promise((resolve, reject) => {
        ...
        pendingResolveRef.current = (d: any) => {
          clearTimeout(t)
          resolve(d)
        }
        pendingRejectRef.current = (er: any) => {
          clearTimeout(t)
          reject(er)
        }
      ...
      if (result.error) {
        ...
        if (userInputRef.current) userInputRef.current.disabled = false
        return
      }

      endGame(result.success === true, result.gain || 0)
  ...
  const skipWord = useCallback(async () => {
    ...
    try {
      const wsData = await sendWebSocketMessage({ bet: currentBet })

      if (wsData.error) {
        setMessage({ text: wsData.error, type: 'error' })
        setAnagramLoading(false)
        return
      }

      if (wsData.shuffled && wsData.hmac) {
        setShuffledWord(wsData.shuffled)
        currentHmacRef.current = wsData.hmac
        setTimeLeft(wsData.time_limit || 30)
        setAnagramLoading(false)
        gameActiveRef.current = true
        startTimer()
  ...
  }, [currentBet, sendWebSocketMessage, startTimer, clearTimer])

  const startGame = useCallback(async () => {
    const bet = parseInt(String(currentBet)) || 0

    if (bet < MIN_BET) {
      setMessage({ text: `La mise minimum est de ${MIN_BET} XOF`, type: 'error' })
      return
    }
    if (bet > balance) {
    ...
    try {
      ...
      await sendBetToServer(bet)

      setIsLoading(true)
      setLoadingMessage('Préparation du mot mystère...')
      ...
      const wsData = await sendWebSocketMessage({ bet: bet })
      ...
      if (wsData.shuffled && wsData.hmac) {
        ...
        throw new Error('Données mot manquantes')
      }
    } catch (error) {
      setMessage({ text: 'Erreur de connexion', type: 'error' })
      await loadBalanceFromServer()
  ...
  }, [currentBet, balance, startTimer, sendWebSocketMessage, clearTimer, sendBetToServer, loadBalanceFromServer])
  ...
  const handleBetChange = useCallback((value: string) => {
    ...
    if (bet < MIN_BET && value !== '') newBet = MIN_BET
    setCurrentBet(newBet)
  }, [])

  const setMaxBet = useCallback(() => {
    handleBetChange(String(balanceRef.current))
  }, [handleBetChange])

  const showBetScreen = useCallback(async () => {
    setIsLoading(true)
    ...
    setCurrentScreen('bet')
    const bet = Math.min(MIN_BET, balance)
    setCurrentBet(bet)
    gameActiveRef.current = false
    currentHmacRef.current = ''
  }, [balance, loadBalanceFromServer])
  ...
  /* ── Effects ────────────────────────────────────────── */
  useEffect(() => {
    if (currentScreen === 'result' && isWin) {
      createConfetti()
    }
  }, [currentScreen, isWin, createConfetti])
  ...
  }, [animateCosmicBackground, clearTimer, loadBalanceFromServer, closeWebSocket])

  useEffect(() => {
    if (currentScreen === 'game' && userInputRef.current) {
      userInputRef.current.focus()
    }
  }, [currentScreen])
  ...
  return (
    <div className="relative min-h-screen font-['Poppins',sans-serif] bg-[#0a0a14] text-white overflow-x-hidden">
      ...
      {/* Loader Overlay */}
      {isLoading && (
        <div className="fixed inset-0 bg-[#0a0e17]/90 backdrop-blur-xl flex flex-col items-center justify-center z-[5000] gap-6">
          <div className="w-20 h-20 bg-gradient-to-br from-[#FF3366] to-[#3399FF] rounded-3xl flex items-center justify-center shadow-[0_0_40px_rgba(255,51,102,0.4)] animate-pulse">
            <LoaderIcon />
          </div>
          ...
          <div className="mt-4 w-48 h-1 bg-white/10 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-[#FF3366] to-[#3399FF] animate-[shimmer_1.5s_ease-in-out_infinite]" style={{ width: '100%' }} />
          </div>
        </div>
      )}
        ...
        <div className="max-w-[600px] mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-2.5">
            <div className="flex items-center gap-2 bg-white/[0.06] py-2 px-4 rounded-full border border-white/[0.08] text-sm font-semibold">
              ...
              <span className="text-white/40 text-xs font-normal">XOF</span>
            </div>
            <button
          ...
          <div className="animate-[screenEnter_0.5s_cubic-bezier(0.22,1,0.36,1)]">
            <div className="bg-white/[0.04] rounded-3xl p-7 sm:p-9 border border-white/[0.08] shadow-[0_8px_32px_rgba(0,0,0,0.4)] relative overflow-hidden backdrop-blur-xl before:absolute before:-top-1/2 before:-left-1/2 before:w-[200%] before:h-[200%] before:bg-[radial-gradient(circle_at_30%_20%,rgba(255,51,102,0.08),transparent_50%),radial-gradient(circle_at_70%_80%,rgba(51,153,255,0.08),transparent_50%)] before:pointer-events-none before:animate-[cardGlowFloat_8s_ease-in-out_infinite]">
              ...
              <div className="text-center mb-7 relative z-10">
                ...
                <p className="text-white/40 text-sm sm:text-base">Choisissez le montant à miser pour cette partie</p>
              </div>
              ...
              <div className="mb-6 relative z-10">
                ...
                <div className="flex justify-between items-center mt-3 text-xs text-white/40">
                  <span>Min: {MIN_BET} XOF</span>
                  <span className="bg-[#FF9933]/15 py-1 px-3 rounded-full text-[#FF9933]">
                    Gain: <strong className="text-white">x{MULTIPLIER}</strong>
                  </span>
                </div>
              </div>
          ...
          <div className="animate-[screenEnter_0.5s_cubic-bezier(0.22,1,0.36,1)]">
            <div className="bg-white/[0.04] rounded-3xl p-6 sm:p-7 border border-white/[0.08] shadow-[0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-xl">
              ...
              <div className="flex justify-between items-center mb-7">
                <div className="flex gap-2.5 flex-wrap">
                  ...
                  <div className="flex items-center gap-1.5 bg-transparent py-2 px-4 rounded-full border border-white/[0.08] text-sm text-white/70">
                    <span>Mise: <strong className="text-white">{formatNumber(currentBet)}</strong> XOF</span>
                  </div>
                </div>
              ...
              <div className={`relative my-6 py-10 px-5 bg-black/30 rounded-2xl border border-white/[0.08] text-center overflow-hidden ${shakeAnagram ? 'animate-[shake_0.5s_ease]' : ''}`}>
                ...
                <div className="text-xs uppercase tracking-[3px] text-white/40 mb-4">Déchiffrez ce mot</div>
                <div className={`text-3xl sm:text-4xl font-extrabold tracking-[6px] sm:tracking-[8px] text-[#3399FF] drop-shadow-[0_0_30px_rgba(51,153,255,0.4)] relative z-10 ${anagramLoading ? 'animate-pulse' : ''}`}>
                  {anagramLoading ? (
                    ...
                    shuffledWord ? shuffledWord.toUpperCase() : 'PRÊT ?'
                  )}
                </div>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={skipWord} className="flex-1 py-4 bg-white/[0.05] text-white/70 border border-white/[0.08] rounded-2xl font-semibold text-base cursor-pointer flex items-center justify-center gap-2 hover:bg-white/[0.1] hover:text-white hover:-translate-y-0.5 transition-all duration-300">
                  <ForwardIcon />
                  Passer
                </button>
              </div>
          ...
          <div className="animate-[screenEnter_0.5s_cubic-bezier(0.22,1,0.36,1)]">
            <div className="bg-white/[0.04] rounded-3xl p-8 sm:p-10 border border-white/[0.08] shadow-[0_8px_32px_rgba(0,0,0,0.4)] text-center relative overflow-hidden backdrop-blur-xl before:absolute before:-top-[30%] before:-left-[30%] before:w-[160%] before:h-[160%] before:bg-[radial-gradient(circle_at_50%_50%,rgba(255,153,51,0.06),transparent_60%)] before:pointer-events-none">
              ...
              <h2 className="text-2xl sm:text-3xl font-extrabold mb-2.5">
                {isWin ? 'Victoire !' : 'Temps écoulé'}
              </h2>

              <p className="text-white/40 text-sm sm:text-base mb-6">
                {isWin
                  ? 'Vous avez découvert le mot mystère !'
                  : 'Dommage, réessayez avec un nouveau mot'
                }
              </p>
              ...
              <div className={`py-5 px-4 rounded-2xl mb-6 flex flex-col items-center gap-1.5 ${isWin ? 'bg-emerald-400/10 border border-emerald-400/20' : 'bg-red-400/10 border border-red-400/20'}`}>
                <span className={`text-3xl sm:text-4xl font-extrabold ${isWin ? 'text-emerald-400 drop-shadow-[0_0_25px_rgba(51,204,102,0.4)]' : 'text-red-400 drop-shadow-[0_0_25px_rgba(255,68,68,0.4)]'}`}>
                  {isWin ? `+${formatNumber(lastGain)}` : `-${formatNumber(currentBet)}`}
                </span>
                <span className="text-xs text-white/40 uppercase tracking-[2px]">XOF</span>
              </div>
              ...
              <div className="flex items-center justify-center gap-5 mb-6 py-4 px-4 bg-black/20 rounded-2xl">
                <div className="text-center">
                  ...
                  <span className="text-lg font-bold text-white">{formatNumber(currentBet)} XOF</span>
                </div>
                <div className="w-px h-8 bg-white/[0.08]" />
                <div className="text-center">
                  <span className="block text-xs text-white/40 uppercase tracking-[1px] mb-1">Multiplicateur</span>
                  <span className="text-lg font-bold text-white">x{MULTIPLIER}</span>
                </div>
              </div>
        ...
        >
          <div className="bg-[#0f0f1e]/98 rounded-3xl p-7 sm:p-9 max-w-[500px] w-full max-h-[85vh] overflow-y-auto border border-white/[0.08] shadow-[0_25px_60px_rgba(0,0,0,0.6)] animate-[modalFadeIn_0.3s_ease]">
            ...
            <div className="flex items-center gap-4 mb-7">
              ...
              <h2 className="flex-1 text-2xl font-bold text-white">Règles du Jeu</h2>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
              {[
                { icon: <CoinsIcon className="!w-5 !h-5" />, title: 'Mise personnalisée', desc: `Choisissez votre mise à partir de ${MIN_BET} XOF. Vous gagnez le double de votre mise en cas de victoire.` },
                { icon: <RandomIcon />, title: 'Anagrammes', desc: 'Un mot mélangé apparaît. Trouvez le mot original en 30 secondes pour gagner.' },
                { icon: <ClockIcon />, title: 'Temps limité', desc: '30 secondes par mot. Si le temps s\'épuise, vous perdez votre mise.' },
                { icon: <ForwardIcon />, title: 'Option Passer', desc: 'Bloqué ? Passez au mot suivant sans pénalité.' }
              ].map((rule, i) => (
