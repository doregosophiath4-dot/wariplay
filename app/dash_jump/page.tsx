...
export default function NeonRunner() {
  ...
  const generateRandomObjective = useCallback((): Objective => {
    const objectives = [
      {
        ...
        name: 'Collecteur',
        difficulties: [
          { coins: 10, difficulty: 'FACILE', description: 'Collecter 10 pieces' },
          { coins: 25, difficulty: 'MOYEN', description: 'Collecter 25 pieces' },
          { coins: 50, difficulty: 'DIFFICILE', description: 'Collecter 50 pieces' }
        ]
      },
      {
        type: 'survive_time' as const,
        name: 'Survivant',
        difficulties: [
          { time: 30, difficulty: 'FACILE', description: 'Survivre 30 secondes' },
          { time: 60, difficulty: 'MOYEN', description: 'Survivre 60 secondes' },
          { time: 90, difficulty: 'DIFFICILE', description: 'Survivre 90 secondes' }
        ]
      },
      {
        type: 'score_in_time' as const,
        name: 'Rapide',
        difficulties: [
          { score: 100, time: 30, difficulty: 'FACILE', description: 'Atteindre 100 points en 30 secondes' },
          { score: 200, time: 40, difficulty: 'MOYEN', description: 'Atteindre 200 points en 40 secondes' },
          { score: 300, time: 45, difficulty: 'DIFFICILE', description: 'Atteindre 300 points en 45 secondes' }
        ]
      },
      {
        type: 'reach_score' as const,
        name: 'Score Master',
        difficulties: [
          { score: 150, difficulty: 'FACILE', description: 'Atteindre 150 points' },
          { score: 300, difficulty: 'MOYEN', description: 'Atteindre 300 points' },
          { score: 500, difficulty: 'DIFFICILE', description: 'Atteindre 500 points' }
        ]
      }
    ]
    ...
    const difficulty = objectiveType.difficulties[difficultyLevel]
    ...
    return {
      ...
      target: difficulty.coins || difficulty.score || difficulty.time || 0,
      timeLimit: (difficulty as any).time || 0,
      progress: 0,
      completed: false
    }
  }, [])
  ...
  const updateObjective = useCallback((timestamp: number) => {
    ...
    if (!obj || gs.objectiveCompleted || gs.objectiveFailed) return
    ...
    switch (obj.type) {
      ...
      case 'score_in_time':
        ...
        progress = score / obj.target
        displayText = `Atteindre ${obj.target} points en ${obj.timeLimit}s: ${score}/${obj.target}`
        if (timeElapsed > obj.timeLimit) {
          gs.objectiveFailed = true
          handleObjectiveFailed()
        }
        break
      case 'reach_score':
        progress = score / obj.target
        displayText = `Atteindre ${obj.target} points: ${score}/${obj.target}`
        break
    }
    ...
    if (progress >= 1 && !gs.objectiveCompleted) {
      gs.objectiveCompleted = true
      handleObjectiveSuccess()
    }
  }, [score])

  const handleObjectiveSuccess = useCallback(() => {
    const win = currentBet * 2
    setUserBalance(prev => prev + win)
    setObjectiveText(`Objectif atteint! +${win} XOF`)
    ...
    setTimeout(() => {
      ...
      if (!gs.gameOver) {
        ...
        setObjectiveResultText(`Objectif atteint! +${win} XOF`)
      }
    }, 2500)
  }, [currentBet])

  const handleObjectiveFailed = useCallback(() => {
  ...
  const endGame = useCallback(() => {
    ...
    if (gs.currentObjective && !gs.objectiveCompleted && !gs.objectiveFailed) {
      handleObjectiveFailed()
    }

    if (gs.currentObjective) {
      if (gs.objectiveCompleted) {
        setObjectiveResultText(`Objectif atteint! +${currentBet * 2} XOF`)
      } else {
  ...
  }, [currentBet, handleObjectiveFailed])
  ...
  }, [score, endGame, generatePlatform, generateCoin, createParticle, updateObjective])
  ...
  const confirmBet = useCallback(() => {
    if (userBalance < currentBet) return
    ...
    setTimeout(() => {
      ...
      gs.currentObjective = obj
      setObjectiveText(obj.description)
      setObjectiveProgress(0)

      setScreen('objective')
      setIsButtonLoading(false)
    }, 5000)
  }, [userBalance, currentBet, generateRandomObjective])
  ...
  useEffect(() => {
    ...
    // Instant jump on touch start / mouse down — NO delay
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setTimeout(() => setScreen('start'), 500)
      }
    ...
    <div ref={containerRef} className="relative w-screen h-screen overflow-hidden bg-[#050510] font-['Orbitron',monospace] text-white touch-none select-none">
      ...
      {screen === 'gameover' && (
        <div className="absolute inset-0 bg-[#050510]/95 backdrop-blur-xl flex flex-col justify-center items-center z-[1000] p-4">
          ...
          <div className={`text-base sm:text-xl mb-5 font-bold ${objectiveResultText.includes('atteint') ? 'text-emerald-400' : 'text-red-400'}`}>
            {objectiveResultText}
          </div>
          <button
