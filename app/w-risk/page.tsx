...
export default function WRiskPage() {
  ...
  const createGrid = useCallback(() => {
    let positiveCount: number, negativeCount: number, bonusCount: number
    switch (currentLevel) {
      case 1: positiveCount = 12; negativeCount = 12; bonusCount = 1; break
      case 2: positiveCount = 10; negativeCount = 13; bonusCount = 2; break
      case 3: positiveCount = 8; negativeCount = 14; bonusCount = 3; break
      default: positiveCount = 12; negativeCount = 12; bonusCount = 1
    }

    const values: number[] = []
    const intervalMin = Math.floor((currentBet * 25) / 100)
    const intervalMax = currentBet

    for (let i = 0; i < positiveCount; i++) {
      values.push(Math.floor(Math.random() * (intervalMax - intervalMin + 1)) + intervalMin)
    }
    for (let i = 0; i < negativeCount; i++) {
      values.push(-Math.floor(Math.random() * (intervalMax - intervalMin + 1)) - intervalMin)
    }
    for (let i = 0; i < bonusCount; i++) {
      values.push(currentBet * 3)
    }

    setGridValues(shuffleArray(values))
    setRevealedCells(new Array(CONFIG.TOTAL_CELLS).fill(false))
    setRevealedCount(0)
    setRevealedValues([])
    setTotalGains(initialBet)
    setScore(initialBet)
  }, [currentLevel, currentBet, initialBet])

  useEffect(() => { createGrid() }, [createGrid])
  ...
  const handleCellClick = (index: number) => {
    ...
    if (revealedCount >= casesToReveal) {
      showNotification(`Vous avez deja revele ${casesToReveal} cases maximum!`, 'warning')
      return
    }
    ...
    if (newRevealedValues.length > 4) newRevealedValues.shift()
    setRevealedValues(newRevealedValues)

    const newTotalGains = totalGains + value
    setTotalGains(newTotalGains)
    ...
    if (newTotalGains <= 0) {
      showNotification("Vos gains sont tombes a 0 ou moins! Fin de la partie.", 'negative')
      setTimeout(() => endGame(), 1500)
      return
    }

    if (newRevealedCount >= casesToReveal) {
  ...
  const startGame = () => {
    ...
    setTotalGains(currentBet)
    setScore(currentBet)
    setRevealedCount(0)
    setRevealedValues([])
    createGrid()
    showNotification(`Partie demarree! Vous pouvez reveler ${casesToReveal} cases maximum avec une mise de ${currentBet} XOF`, 'positive')
  }

  const collectGains = () => {
    if (revealedCount < 2 || totalGains <= 0) {
      showNotification("Vous devez reveler au moins 2 cases pour encaisser", 'warning')
      return
    }
    if (totalGains > 0) {
      setWalletBalance(prev => prev + totalGains)
      showNotification(`Vous avez encaisse ${totalGains} XOF!`, 'positive')
      setGameActive(false)
  ...
  const endGame = () => {
    ...
    setModal({
      visible: true,
      title: score > initialBet ? 'Felicitations !' : 'Dommage !',
      message: score > initialBet
        ? `Vous avez gagne ${score.toLocaleString()} XOF en revelant ${revealedCount} cases`
        : `Votre score est de ${score.toLocaleString()} XOF apres ${revealedCount} cases revelees`,
      isRules: false
    })
  }
  ...
  /* ── Bet Controls ───────────────────────────────────── */
  const increaseBet = () => { if (currentBet < 1000) setCurrentBet(prev => prev + (prev < 500 ? 100 : 500)) }
  const decreaseBet = () => { if (currentBet > 100) setCurrentBet(prev => prev - (prev <= 500 ? 100 : 500)) }
  const increaseCases = () => { if (casesToReveal < 25) setCasesToReveal(prev => prev + 1) }
  const decreaseCases = () => { if (casesToReveal > 3) setCasesToReveal(prev => prev - 1) }
  ...
  const isPositive = (index: number) => revealedCells[index] && gridValues[index] > 0 && gridValues[index] < initialBet * 3
  const isNegative = (index: number) => revealedCells[index] && gridValues[index] < 0
  const isBonus = (index: number) => revealedCells[index] && gridValues[index] >= initialBet * 3
      ...
      <div className="relative z-10 w-full max-w-[720px] flex flex-col items-center gap-5 px-2">
        ...
        <div className="w-full flex justify-center">
          <div className="flex gap-2.5 bg-gradient-to-b from-[#111b2b] to-[#0c131f] border border-orange-400/20 rounded-2xl py-3 px-4 sm:px-5 flex-wrap justify-center w-full shadow-[0_12px_35px_rgba(0,0,0,0.65),0_0_25px_rgba(255,140,0,0.3)] relative overflow-hidden
            ...
            <button className="flex items-center gap-1.5 bg-gradient-to-b from-[#161b22]/95 to-[#10141c]/90 text-white/90 border border-white/[0.06] rounded-xl py-2 px-4 text-xs cursor-pointer shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_3px_8px_rgba(0,0,0,0.5)] hover:bg-orange-400/10 hover:shadow-[0_0_25px_rgba(255,140,0,0.3)] hover:-translate-y-0.5 active:translate-y-0 transition-all duration-300">
              <WalletIcon />
              <span>{walletBalance.toLocaleString()} XOF</span>
            </button>
          ...
          <div className="grid grid-cols-5 gap-2 sm:gap-2.5 bg-[radial-gradient(ellipse_at_center,#111b2b_0%,#080c14_100%)] p-4 sm:p-5 rounded-3xl w-full max-w-[480px] border border-white/[0.04] shadow-[inset_0_2px_8px_rgba(0,0,0,0.6),inset_0_-1px_0_rgba(255,255,255,0.03),0_10px_30px_rgba(0,0,0,0.6),0_0_50px_rgba(255,140,0,0.06)] relative
            ...
            {Array.from({ length: CONFIG.TOTAL_CELLS }).map((_, i) => (
              <motion.div
                ...
                whileHover={!revealedCells[i] ? { scale: 1.04 } : {}}
                whileTap={!revealedCells[i] ? { scale: 0.96 } : {}}
                className={`
                  ...
                  ${revealedCells[i] ? 'pointer-events-none' : ''}
                ...
                `}
                style={{ transform: revealedCells[i] ? 'perspective(500px) rotateX(2deg)' : 'perspective(500px) rotateX(2deg)' }}
              >
          ...
          {!showFuturistic && (
            ...
            >
              ...
              <div className="flex items-center gap-2.5 justify-center flex-wrap">
                ...
                <button
                  onClick={startGame}
                  disabled={walletBalance < currentBet}
            ...
            >
              ...
              <div className="flex items-center gap-4 bg-gradient-to-b from-[#0f1826] to-[#0a1018] rounded-3xl p-5 flex-wrap shadow-[0_12px_35px_rgba(0,0,0,0.65),0_0_25px_rgba(255,140,0,0.3)] border border-orange-400/20 relative overflow-hidden
                ...
                <div className="flex items-center gap-3.5 bg-gradient-to-b from-[#131f2e] to-[#0c131f] rounded-2xl py-4 px-5 flex-1 justify-center shadow-[inset_0_1px_0_rgba(255,255,255,0.08),inset_0_-4px_8px_rgba(0,0,0,0.4)] border border-white/[0.06]">
                  <button onClick={decreaseBet} disabled={currentBet <= 100} className="w-12 h-12 rounded-2xl bg-gradient-to-b from-[#1a2a3d] to-[#0f1927] text-gray-300 border border-white/[0.06] text-2xl font-bold cursor-pointer shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_4px_10px_rgba(0,0,0,0.5)] disabled:opacity-35 disabled:cursor-not-allowed hover:bg-gradient-to-b hover:from-[#263850] hover:to-[#162030] hover:text-white hover:-translate-y-0.5 transition-all duration-300">−</button>
                  <div>
                  ...
                  <button onClick={increaseBet} disabled={currentBet >= 1000} className="w-12 h-12 rounded-2xl bg-gradient-to-b from-[#1a2a3d] to-[#0f1927] text-gray-300 border border-white/[0.06] text-2xl font-bold cursor-pointer shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_4px_10px_rgba(0,0,0,0.5)] disabled:opacity-35 disabled:cursor-not-allowed hover:bg-gradient-to-b hover:from-[#263850] hover:to-[#162030] hover:text-white hover:-translate-y-0.5 transition-all duration-300">+</button>
                </div>

                <button
                  onClick={collectGains}
                  disabled={revealedCount < 2 || totalGains <= 0}
                ...
                >
                  ...
                  <span className="text-sm uppercase tracking-[0.5px]">Encaisser</span>
                </button>
              </div>
            </motion.div>
          )}
