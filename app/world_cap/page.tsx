...
const DATA: CountryData[] = [
  ...
  { country: "Bresil", capital: "Brasilia" },
  ...
  { country: "Algerie", capital: "Alger" },
  { country: "Portugal", capital: "Lisbonne" },
  { country: "Royaume-Uni", capital: "Londres" },
  { country: "Etats-Unis", capital: "Washington" },
  { country: "Coree du Sud", capital: "Seoul" },
  { country: "Vietnam", capital: "Hanoi" },
  { country: "Thailande", capital: "Bangkok" },
  { country: "Indonesie", capital: "Jakarta" },
  ...
  { country: "Suede", capital: "Stockholm" },
  { country: "Norvege", capital: "Oslo" },
...
export default function WorldCap() {
  ...
  const playerBalanceRef = useRef(10000)
  ...
  /* ── Game Logic ─────────────────────────────────────── */
  const checkCriticalLoss = useCallback((): boolean => {
    const bet = currentBetRef.current
    const original = originalBetRef.current
    if (bet <= 0) { showGameOverPopup(false, "Vous avez perdu toute votre mise !"); return true }
    const lossPercentage = ((original - bet) / original) * 100
    if (lossPercentage >= 40) { showGameOverPopup(false, `Votre mise a diminue de ${lossPercentage.toFixed(1)}% (plus de 40%) !`); return true }
    return false
  }, [])

  const checkMaxWrongAnswers = useCallback((): boolean => {
    if (wrongCountRef.current >= MAX_WRONG_ANSWERS) {
      showGameOverPopup(false, `Vous avez ${wrongCountRef.current} mauvaises reponses sur ${MAX_QUESTIONS} questions !`)
      return true
    }
    return false
  }, [])

  const checkLastQuestion = useCallback((): boolean => {
    if (currentQuestionRef.current >= MAX_QUESTIONS) {
      showGameOverPopup(true, "Felicitations ! Vous avez termine les 20 questions !")
      return true
    }
    return false
  }, [])
  ...
  const timeOut = useCallback(() => {
    const lossAmount = Math.floor(currentBetRef.current * 0.2)
    setCurrentBet(prev => { const nb = Math.max(0, prev - lossAmount); currentBetRef.current = nb; return nb })
    setWrongCount(prev => { const nw = prev + 1; wrongCountRef.current = nw; return nw })
    setStreak(0)
    streakRef.current = 0
    showNotificationFn("Temps ecoule !", `Vous avez perdu ${formatNumber(lossAmount)} XOF`, "error")
    if (checkCriticalLoss() || checkMaxWrongAnswers()) return
    nextQuestion()
  }, [showNotificationFn, formatNumber, checkCriticalLoss, checkMaxWrongAnswers])
  ...
  const nextQuestion = useCallback(() => {
    ...
    autoNextRef.current = setTimeout(() => {
      setCurrentQuestion(prev => {
        ...
        if (checkLastQuestion()) return newQ
        setTimeout(() => newQuestionFn(), 50)
        return newQ
      })
    }, 2000)
  }, [checkLastQuestion])

  const newQuestionFn = useCallback(() => {
    if (autoNextRef.current) clearTimeout(autoNextRef.current)
    ...
    while (opts.length < 4) {
      ...
      const wrongCapital = randomCountry.capital
      if (!opts.includes(wrongCapital) && wrongCapital !== answer) opts.push(wrongCapital)
    }
  ...
  const handleAnswer = useCallback((selected: string) => {
    ...
    setCorrectChoice(currentAnswerRef.current)

    if (selected === currentAnswerRef.current) {
      ...
      if (streakRef.current > maxStreakRef.current) { setMaxStreak(streakRef.current); maxStreakRef.current = streakRef.current }
      const winAmount = Math.floor(currentBetRef.current * 0.2)
      setCurrentBet(prev => { const nb = prev + winAmount; currentBetRef.current = nb; return nb })
      showNotificationFn("Bonne reponse !", `+${formatNumber(winAmount)} XOF`, "success")
    } else {
      setStreak(0)
      streakRef.current = 0
      setWrongCount(prev => { const nw = prev + 1; wrongCountRef.current = nw; return nw })
      const lossAmount = Math.floor(currentBetRef.current * 0.2)
      setCurrentBet(prev => { const nb = Math.max(0, prev - lossAmount); currentBetRef.current = nb; return nb })
      showNotificationFn("Mauvaise reponse", `-${formatNumber(lossAmount)} XOF`, "error")
    }

    if (checkCriticalLoss() || checkMaxWrongAnswers()) return
    nextQuestion()
  }, [clearTimer, showNotificationFn, formatNumber, checkCriticalLoss, checkMaxWrongAnswers, nextQuestion])

  const handleStartGame = useCallback(() => {
    const betValue = parseInt(betInputRef.current?.value ?? '')
    if (isNaN(betValue) || betValue < 100 || betValue > 10000) { showNotificationFn("Mise invalide", "Veuillez entrer un montant entre 100 et 10 000 XOF", "error"); betInputRef.current?.focus(); return }
    if (betValue > playerBalanceRef.current) { showNotificationFn("Solde insuffisant", `Votre solde est de ${formatNumber(playerBalanceRef.current)} XOF`, "error"); return }

    setPlayerBalance(prev => { const nb = prev - betValue; playerBalanceRef.current = nb; return nb })
    setOriginalBet(betValue)
    originalBetRef.current = betValue
    ...
    newQuestionFn()
    showNotificationFn("Bonne chance !", `Vous avez mise ${formatNumber(betValue)} XOF sur 20 questions`, "info")
  }, [newQuestionFn, showNotificationFn, formatNumber])
  ...
  useEffect(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    const resizeCanvas = () => { canvas.width = window.innerWidth; canvas.height = window.innerHeight; initParticles(canvas.width, canvas.height); initStars(canvas.width, canvas.height) }
    resizeCanvas()
    window.addEventListener('resize', resizeCanvas)
    animate()
    const handleKeyDown = (e: KeyboardEvent) => { if (e.key === 'Escape') { setShowRules(false); setShowGameOver(false); handleEndGame() } }
    document.addEventListener('keydown', handleKeyDown)
    return () => { window.removeEventListener('resize', resizeCanvas); document.removeEventListener('keydown', handleKeyDown); cancelAnimationFrame(animFrameRef.current); clearTimer(); if (autoNextRef.current) clearTimeout(autoNextRef.current); if (notificationTimeoutRef.current) clearTimeout(notificationTimeoutRef.current) }
  }, [animate, initParticles, initStars, clearTimer, handleEndGame])

  const progressPercent = (currentQuestion / MAX_QUESTIONS) * 100
      ...
      <div className="w-full max-w-[900px] mx-auto px-4 sm:px-5 py-4 relative z-10">
        ...
        <header className={`text-center mb-5 pt-2.5 transition-all duration-400 ${gameActive ? 'opacity-0 invisible h-0 mb-0 p-0' : 'opacity-100 visible'}`}>
          ...
          <p className="text-sm sm:text-base text-white/80 max-w-[600px] mx-auto">
            Testez vos connaissances geographiques, misez et gagnez du XOF!
          </p>
        </header>
        ...
        <section className={`bg-white/95 backdrop-blur-xl rounded-2xl p-5 sm:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.15)] mb-5 border border-purple-500/20 hover:-translate-y-1 hover:shadow-[0_25px_60px_rgba(108,99,255,0.15)] transition-all duration-400 ${gameActive ? 'hidden' : 'block'}`}>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-gray-500 text-xs sm:text-sm">Mise minimale : 100 XOF | Mise maximale : 1000 XOF</p>
            </div>
        ...
        <section className={`bg-white/95 backdrop-blur-xl rounded-2xl p-4 sm:p-6 shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-purple-500/20 hover:-translate-y-1 hover:shadow-[0_25px_60px_rgba(108,99,255,0.15)] transition-all duration-400 mt-[70px] ${!gameActive ? 'hidden' : 'block'}`}>
          ...
          <div className="my-4 bg-white/[0.1] rounded-xl p-2">
            <div className="flex justify-between mb-1.5 text-white text-sm">
              <span>Question {currentQuestion}/{MAX_QUESTIONS}</span>
          ...
          <div className="mb-5">
            <div className="flex overflow-x-auto gap-3 pb-2.5 scrollbar-thin">
              {[
                ...
                { value: streak, label: 'Serie en cours', color: 'text-pink-400' },
                { value: correctCount, label: 'Bonnes reponses', color: 'text-emerald-400' },
                { value: wrongCount, label: 'Mauvaises reponses', color: 'text-red-400' },
      ...
      <div className={`fixed inset-0 bg-black/90 backdrop-blur-xl z-[2000] flex items-center justify-center p-4 transition-all duration-300 ${showGameOver ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        <div className={`bg-white rounded-2xl p-6 sm:p-8 max-w-[500px] w-full max-h-[80vh] overflow-y-auto shadow-[0_25px_50px_rgba(0,0,0,0.4)] text-center transition-all duration-400 ${showGameOver ? 'translate-y-0' : 'translate-y-8'}`}>
          ...
          <h2 className="text-purple-500 text-2xl sm:text-3xl font-bold mb-4">
            {gameOverWin ? 'Felicitations !' : 'Partie terminee !'}
          </h2>
          ...
          <div className="grid grid-cols-2 gap-4 my-6">
            {[
              ...
              { value: correctCount, label: 'Bonnes reponses' },
              { value: wrongCount, label: 'Mauvaises reponses' },
              { value: maxStreak, label: 'Meilleure serie' }
            ].map((stat, i) => (
        ...
        <div className={`bg-white rounded-2xl p-5 sm:p-6 max-w-[500px] w-full max-h-[80vh] overflow-y-auto shadow-[0_25px_50px_rgba(0,0,0,0.3)] transition-all duration-400 ${showRules ? 'translate-y-0' : 'translate-y-8'}`}>
          ...
          <div className="text-[#2D3047] text-sm">
            ...
            <ul className="pl-4 mb-4 space-y-2">
              ...
              <li>2. <span className="text-purple-500 font-semibold">Devinez la capitale</span> du pays qui vous est propose</li>
              <li>3. <span className="text-purple-500 font-semibold">Vous avez 8 secondes</span> pour repondre a chaque question</li>
              <li>4. <span className="text-purple-500 font-semibold">Choisissez parmi 4 options</span> de reponses possibles</li>
              <li>5. <span className="text-purple-500 font-semibold">20 questions maximum</span> par partie</li>
            </ul>

            <h3 className="text-purple-500 text-base font-bold my-3">Systeme de gains et pertes</h3>
            <ul className="pl-4 mb-4 space-y-2">
              <li className="flex items-center gap-1.5"><TrophyIcon className="!w-4 !h-4 text-emerald-400" /> <span className="text-purple-500 font-semibold">Bonne reponse</span> : +20% de votre mise actuelle</li>
              <li className="flex items-center gap-1.5"><CloseModalIcon /> <span className="text-purple-500 font-semibold">Mauvaise reponse</span> : -20% de votre mise actuelle</li>
              <li className="flex items-center gap-1.5"><ClockIcon /> <span className="text-purple-500 font-semibold">Temps ecoule</span> : -20% de votre mise actuelle</li>
              <li className="flex items-center gap-1.5"><WarningIcon /> <span className="text-purple-500 font-semibold">Perte critique</span> : Si votre mise descend a -40% de la mise initiale, vous perdez tout</li>
              <li className="flex items-center gap-1.5"><CloseIcon className="!w-4 !h-4" /> <span className="text-purple-500 font-semibold">Condition de defaite</span> : Si vous avez 10 mauvaises reponses sur 20 questions, vous perdez tout</li>
            </ul>
            ...
            <ul className="pl-4 mb-4 space-y-1.5">
              ...
              <li>• Apres 1 bonne reponse : 1 200 XOF (+20%)</li>
              <li>• Apres 1 mauvaise reponse : 960 XOF (-20% de 1 200)</li>
              <li>• Si la mise descend en dessous de 600 XOF (-40% de 1 000) : Perte immediate</li>
              <li>• Si 10 mauvaises reponses sur 20 questions : Perte immediate</li>
            </ul>
            ...
            <ul className="pl-4 mb-4 space-y-1.5">
              ...
              <li>• Surveillez le timer en haut a gauche</li>
              <li>• Essayez de maintenir un bon ratio de bonnes reponses</li>
              <li>• Ne laissez pas votre mise descendre trop bas</li>
              <li>• Le jeu se termine automatiquement apres 20 questions</li>
            </ul>
